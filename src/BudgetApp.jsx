...
import { Wallet, TrendingUp, Plus, History, LayoutDashboard, Edit2, Trash2, Search, Filter } from 'lucide-react';
...
const BudgetApp = () => {
  ...
  const [newExpense, setNewExpense] = useState({ amount: '', category: 'Alimentation', date: new Date().toISOString().split('T')[0], note: '' });
  const [editingExpense, setEditingExpense] = useState(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [filterCategory, setFilterCategory] = useState('all');
  const [budgetForm, setBudgetForm] = useState({ monthly: '', categories: {} });
  ...
  const handleSetBudget = (e) => {
    e.preventDefault();
    const monthly = parseFloat(budgetForm.monthly);
    const newBudget = { monthly, categories: {} };
    setBudget(newBudget);
  ...
  if (!user) return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-black p-4">
      <div className="bg-gray-800 p-8 rounded-3xl w-full max-w-md space-y-6 border border-gray-700">
        {currentView === 'login' ? (
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-green-500 py-3 rounded-xl text-white font-bold hover:bg-green-600 transition">Se connecter</button>
          ...
          <form onSubmit={handleSignUp} className="space-y-4">
            ...
            <input type="email" name="email" placeholder="Email" className="w-full p-3 rounded-xl bg-gray-700 text-white" required />
            <input type="password" name="password" placeholder="Mot de passe" className="w-full p-3 rounded-xl bg-gray-700 text-white" required />
            <button type="submit" className="w-full bg-green-500 py-3 rounded-xl text-white font-bold hover:bg-green-600 transition">Créer un compte</button>
  ...
  if (currentView === 'profil') return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-gray-900 via-gray-800 to-black">
      <form onSubmit={handleUpdateProfile} className="bg-gray-800 p-8 rounded-3xl w-full max-w-md border border-gray-700 space-y-6">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black p-4">
      ...
      {/* Dashboard simple avec budget & dépenses */}
      {!budget ? (
        <form onSubmit={handleSetBudget} className="bg-gray-800 p-6 rounded-2xl space-y-4 max-w-md">
          <h2 className="text-white font-bold text-lg">Définir votre budget mensuel</h2>
          <input type="number" placeholder="Budget mensuel (€)" value={budgetForm.monthly} onChange={e=>setBudgetForm({...budgetForm, monthly:e.target.value})} className="w-full p-3 rounded-xl bg-gray-700 text-white"/>
          <button type="submit" className="w-full py-3 bg-green-500 rounded-xl text-white font-bold hover:bg-green-600 transition">Valider</button>
        </form>
      ) : (
        <div className="space-y-6">
          <h2 className="text-white font-bold text-lg">Budget mensuel : {budget.monthly} €</h2>
          <h3 className="text-white font-bold">Total dépenses : {getTotalExpenses()} €</h3>

          <form onSubmit={handleAddExpense} className="bg-gray-800 p-4 rounded-2xl space-y-2 max-w-md">
